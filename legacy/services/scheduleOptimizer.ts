...
export function formatFreeSlots(slots: TimeSlot[]): string {
  if (slots.length === 0) return '遨ｺ縺肴凾髢薙↑縺暦ｼ域里蟄倅ｺ亥ｮ壹〒蝓九∪縺｣縺ｦ縺・∪縺呻ｼ・;
  return slots
    .map((s) => {
      const mins = s.end - s.start;
      return `${minutesToTime(s.start)}-${minutesToTime(s.end)}・・{mins}蛻・ｼ荏;
    })
    .join('\n');
}
...
function scorePlacement(task: ParsedTask, startMinutes: number): number {
  ...
  // 菴主━蜈亥ｺｦ: 繧ｫ繝・ざ繝ｪ縺ｮ逅・Φ譎る俣蟶ｯ繧帝∩縺代√〒縺阪ｋ縺縺鷹≦縺・凾髢灘ｸｯ縺ｸ
  if (priority >= 4) {
    return startMinutes + (inIdeal ? -300 : 0);
  }

  // 鬮伜━蜈亥ｺｦ: 繧ｫ繝・ざ繝ｪ縺ｮ逅・Φ譎る俣蟶ｯ繧呈怙蜆ｪ蜈・  if (priority <= 2) {
  ...
  // 譎ｮ騾・ 逅・Φ譎る俣蟶ｯ繧貞ｼｱ縺丞━蜈・  const fitScore = inIdeal ? 80 : -Math.abs(mid - prefMid) * 0.5;

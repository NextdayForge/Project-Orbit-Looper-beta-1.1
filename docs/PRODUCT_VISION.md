...
### 新規アイデアの審議（2026-07-06・claude.aiとの対話を経て）

ユーザーがclaude.aiに相談した3つの新アイデア（① タスクをドラッグで並び替え、② タスクを「笑えるほど小さいステップ」に分解、③ 音声入力）を、実コードの状態と突き合わせて審議した。

**claude.aiの評価（要約）:** ②を最優先（着手障壁を下げ実行フィデリティの穴を直接埋めうる。ただしGemini依存でローカルファースト原則から外れるためPro機能向き）、①は検証フェーズ後（実装コストが重く、今は手を広げる余裕がない。副次的に「手動並び替え＝AI判断が外れた信号」として学習に使える可能性はある）、③は保留（差別化と無関係、クラウドAPI依存でローカルファーストと相性が悪い）。

**実コード確認で判明した点:**
- ②はゼロからの新規機能。`Task`型にchecklist/subtask相当のフィールドは無く、既存の`splittable`（時間ベースのセッション分割）とは全くの別物であり混同しないこと。
- 計測（優先順位4）は依然ゼロ、Task Proposal Engine（優先順位3）も未完のまま。この2つが②より前に並んでいる意味は大きい: **計測が無い状態で②を作ると、着手障壁を下げる効果が本当にあったのか検証できないまま終わる**（「作るより検証」という本フェーズの方針そのものに反する）。
- ②の狙い（着手障壁を下げる）自体には、Gemini呼び出し・ドメイン変更・Pro課金設計を伴わない**軽量な代替**が既にある: Today画面に「今すぐ始める」ボタンを目立たせる等のUI動線強化（優先順位1に literally 該当し、今すぐ着手できる）。
- ②をやる場合も、チェックリストをTask側のメタデータとして持たせれば、設計原則1（Task=やること・不変 / Session=いつやるか）やPlacementエンジンには影響しない設計にできる。

**収束した並び順:**
1. 計測導入（優先順位4）— 以降の全施策の効果測定に必須、最優先
2. 動線強化・軽量版（優先順位1の続き）— Gemini不要でマイクロステップの仮説を安く先に検証
3. マイクロステップ分解（Pro機能）— 2で仮説が裏付いてから、課金設計（優先順位6）とセットで
4. ドラッグ並び替え — 検証フェーズ完了後
5. 音声入力 — 保留

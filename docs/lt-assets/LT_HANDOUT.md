...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/9ba7eada-4bf8-4548-b401-56f1ff71f0ef) | ![](qr-android.png) |

> ⚠️ APKは 2026-09-03 の再ビルド（`9ba7eada…`）が最新。7/07ビルド（`dc583766…`）以降の全変更に加え、**毎朝の起床時通知（DAILYトリガー・ヘッドアップ表示）**と**計測（設定→ベータ→計測レポートでタイマー開始率などを確認可能）**が新規に含まれる。それより前のビルドは配布しないこと。（実機/エミュレータでの動作確認済み: 2026-09-05）
...
- [ ] Android実機で**新APK（`9ba7eada…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）
- [ ] 設定→ベータ→計測レポートが表示され、タスク登録→タイマー開始→完了でタイマー開始率が動くか確認
- [ ] 起床時刻を数分後に設定し、DAILY通知がヘッドアップ表示されるか確認

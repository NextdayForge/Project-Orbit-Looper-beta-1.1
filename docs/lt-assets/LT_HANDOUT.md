...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/bd617755-295b-4011-9d08-d0a2c2270a69) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-04 の再ビルド（`bd617755…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定（全角！対応）・解釈結果プレビュー・予定編集モーダルのスクロール修正・モーダルのキーボード被り修正・**配置場所が無くなったタスクが行方不明になるバグの修正**がすべて含まれる。それより前のビルドは配布しないこと。
...
- [ ] Android実機で**新APK（`bd617755…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）

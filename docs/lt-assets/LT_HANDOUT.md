...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/dc583766-9c0d-4d11-b680-8dbcc811c01d) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-07 の再ビルド（`dc583766…`）が最新。従来の修正（繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定・予定編集モーダルのスクロール修正・モーダルのキーボード被り修正・配置場所が無くなったタスクが行方不明になるバグ修正・ボトムシートのタップ閉じ・ふりかえりの保存/キャンセルの固定フッター化・タスク削除が達成率を永久に下げてしまうバグの修正・削除したはずのタスクがAIの再配置で復活するバグの修正）に加え、**Geminiプロキシの防御強化（モデル許可リスト・レート制限）とローカルデータ破損時のフォールバック（内部堅牢性の修正、テスターが操作で確認できる項目ではない）**が含まれる。それより前のビルドは配布しないこと。
...
- [ ] Android実機で**新APK（`dc583766…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）
- [ ] 「今日のふりかえり」で、スクロールせずに保存/キャンセルボタンが押せるか確認
- [ ] AIスケジュール作成の一括入力で「!高／！最高」等の優先度指定が「解釈結果」に反映されるか確認
- [ ] 予定編集モーダルで、スクロールしなくても保存・削除・キャンセルが押せるか確認
- [ ] AIコーチのチャット入力時、キーボードで入力欄が隠れないか確認
- [ ] ボトムシートのハンドルバーを**タップ**して閉じられるか確認（ドラッグが効かなくてもタップは確実に閉じる想定）
...
- [ ] タスクを編集画面から削除し、今日の進捗率・総タスク数が正しく減る（削除したタスクが分母に残らない）か確認
- [ ] 複数回に分かれたタスクの一部を削除し、「今日の予定をAIで作り直す」を実行しても削除した分が再び作成されないか確認

...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/c730ba40-2043-487c-9ea6-39860a88255e) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-04 の再ビルド（`c730ba40…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定（!高）・詳細設定ボタン視認性改善・**Google AI Studioリンクの改善（Web版UIのみ、APK自体には実質影響なし）**がすべて含まれる。それより前のビルド（`ac3df568…`／`f082f12a…`／`13d16f12…`／`a5866f87…`／`e4b08ae5…`）は配布しないこと。
...
- [ ] Android実機で**新APK（`c730ba40…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）

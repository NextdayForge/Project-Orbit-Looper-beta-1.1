...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/219eabbb-22cf-4864-8eda-7b754138ed9d) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-06 の再ビルド（`219eabbb…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定（全角！対応）・解釈結果プレビュー・予定編集モーダルのスクロール修正・モーダルのキーボード被り修正・配置場所が無くなったタスクが行方不明になるバグ修正・**ボトムシートをタップでも閉じられる機能を追加**（ドラッグの実機不具合の代替対応。バーに「スワイプ / タップで閉じる」の案内表示あり）がすべて含まれる。それより前のビルドは配布しないこと。
...
- [ ] Android実機で**新APK（`219eabbb…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）

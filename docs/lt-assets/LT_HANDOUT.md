...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/f082f12a-7f51-4c62-a785-decbbde449fa) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-04 の再ビルド（`f082f12a…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・**AI全再計画で明日のタスクが複製される不具合の修正**がすべて含まれる。それより前のビルド（`13d16f12…`／`a5866f87…`／`e4b08ae5…`）は配布しないこと。
...
- [ ] Android実機で**新APK（`f082f12a…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）

...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/e43fd930-f03b-433d-90fd-43569dbae6ec) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-04 の再ビルド（`e43fd930…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定（全角！対応）・解釈結果プレビュー・予定編集モーダルのスクロール修正・モーダルのキーボード被り修正・配置場所が無くなったタスクが行方不明になるバグ修正・**ボトムシートのドラッグ閉じ（locationY判定を廃止した根本修正版）**がすべて含まれる。それより前のビルド（`8949c690…`含む）は配布しないこと。

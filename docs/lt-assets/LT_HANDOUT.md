...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/97b6374a-d966-4c8b-be90-3849d88872b9) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-04 の再ビルド（`97b6374a…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定（!高／全角！対応）・解釈結果プレビュー・**予定編集モーダルのスクロール不能修正（保存/削除/キャンセルが常時表示）**がすべて含まれる。それより前のビルドは配布しないこと。
...
- [ ] Android実機で**新APK（`97b6374a…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）

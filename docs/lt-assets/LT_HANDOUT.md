...
| **Android APK** | Androidのみ（通知など実機機能が使える） | [インストールページ](https://expo.dev/accounts/asuforge/projects/orbit-looper/builds/72a7e82f-9cb0-496d-b6a5-274ced2d55f7) | ![](qr-android.png) |

> ⚠️ APKは 2026-07-06 の再ビルド（`72a7e82f…`）が最新。これには繰り越し強化・フォーカスタイマー修正・再インストール時のデータ残存修正・AI全再計画の複製バグ修正・一括入力の優先度指定（全角！対応）・解釈結果プレビュー・予定編集モーダルのスクロール修正・モーダルのキーボード被り修正・配置場所が無くなったタスクが行方不明になるバグ修正・ボトムシートをタップでも閉じられる機能（確認済み）・ふりかえりの保存/キャンセルを固定フッター化・タスク削除が達成率を永久に下げてしまうバグの修正・**削除したはずのタスクがAIの再配置で復活するバグの修正**がすべて含まれる。それより前のビルドは配布しないこと。
...
- [ ] Android実機で**新APK（`72a7e82f…`）**をインストール〜起動〜AIコーチ応答まで1回通す（旧APKは配布しない）

      ...
      <View style={styles.days}>
        {grid.map((date) => {
          ...
          return (
            <TouchableOpacity
              key={key}
              style={[styles.day, isToday && styles.daySelected]}
              ...
              {count > 0 && (
                <View style={styles.dots}>
                  {Array.from({ length: Math.min(count, 3) }).map((_, i) => (
                    <View key={i} style={[styles.dot, isToday && styles.dotSelected]} />
                  ))}
                </View>
              )}
            </TouchableOpacity>
...
const makeStyles = (theme: Theme) =>
  ...
  dayNumSelected: { color: theme.onAccent, fontWeight: '700' },
  ...
  dotSelected: { backgroundColor: theme.onAccent },
  });

...
Claude Code 用のプロジェクト指示書。`.cursorrules`（設計思想）を引き継ぎ、日々の開発運用の情報を追加したもの。**このファイルは毎セッション読み込まれるので、簡潔・高信号を保つこと。**
...
詳細な設計背景は [.cursorrules](.cursorrules) を参照（Cursor 用だが設計ドキュメントとしてまだ有効）。
